...
export type ConfigSchema = ReturnType<typeof OpenHearthSchema.toJSONSchema>;
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = OpenHearthSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "OpenHearthConfig";
  const hints = mapSensitivePaths(OpenHearthSchema, "", buildBaseHints());
  const next = {
