...
export class HearthMemberRegistry {
  ...
  constructor(stateDir: string) {
    const hearthDir = path.join(stateDir, "hearth");
    if (!fsSync.existsSync(hearthDir)) {
      fsSync.mkdirSync(hearthDir, { recursive: true });
    }
    this.dbPath = path.join(hearthDir, "members.db");
