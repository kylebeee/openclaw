...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openhearth.sandbox=1");
  args.push("--label", `openhearth.sessionKey=${params.scopeKey}`);
  args.push("--label", `openhearth.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openhearth.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("openhearth.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openhearth sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openhearth sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openhearth sandbox recreate --all");
}
