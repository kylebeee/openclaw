...
declare const __OPENHEARTH_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __OPENHEARTH_VERSION__ === "string" && __OPENHEARTH_VERSION__) ||
  process.env.OPENHEARTH_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `openhearth --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.OPENHEARTH_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.OPENHEARTH_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[openhearth] Gateway daemon failed:",
