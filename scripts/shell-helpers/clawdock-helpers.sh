#!/usr/bin/env bash
# ClawDock - Docker helpers for OpenHearth
# Inspired by Simon Willison's "Running OpenHearth in Docker"
# https://til.simonwillison.net/llms/openhearth-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/kylebeee/openhearth/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for OpenHearth
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/openhearth"
  "${HOME}/workspace/openhearth"
  "${HOME}/projects/openhearth"
  "${HOME}/dev/openhearth"
  "${HOME}/code/openhearth"
  "${HOME}/src/openhearth"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found OpenHearth at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/openhearth"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OpenHearth not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/kylebeee/openhearth.git ~/openhearth"
    echo "  cd ~/openhearth && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OPENHEARTH_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d openhearth-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart openhearth-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f openhearth-gateway
}
...
clawdock-workspace() {
  cd ~/.openhearth/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec openhearth-gateway \
    bash -c 'echo "alias openhearth=\"./openhearth.mjs\"" > /tmp/.bashrc_openhearth && bash --rcfile /tmp/.bashrc_openhearth'
}
...
clawdock-cli() {
  _clawdock_compose run --rm openhearth-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build openhearth-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENHEARTH_GATEWAY_TOKEN=$token" openhearth-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" openhearth-gateway \
    bash -c './openhearth.mjs config set gateway.remote.token "$TOKEN" && ./openhearth.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec openhearth-gateway \
    bash -c "./openhearth.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart openhearth-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm openhearth-cli dashboard --no-open 2>&1)
  status=$?
...
clawdock-devices() {
  ...
  local output status
  output=$(_clawdock_compose exec openhearth-gateway node dist/index.js devices list 2>&1)
  status=$?
  ...
  if [ $status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'openhearth config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for OpenHearth${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (openhearth alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to openhearth project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.openhearth)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'openhearth channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'openhearth status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.openhearth.ai${_CLR_RESET}"
  echo ""
}
