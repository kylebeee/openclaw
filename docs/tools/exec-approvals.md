...
- **gateway host** → `openhearth` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openhearth/exec-approvals.sock",
...
`~/.openhearth/exec-approvals.json` directly.

CLI: `openhearth approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
