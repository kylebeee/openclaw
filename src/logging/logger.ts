...
import type { OpenHearthConfig } from "../config/types.js";
import type { ConsoleStyle } from "./console.js";
import { resolvePreferredOpenHearthTmpDir } from "../infra/tmp-openhearth-dir.js";
import { readLoggingConfig } from "./config.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredOpenHearthTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "openhearth.log"); // legacy single-file path

const LOG_PREFIX = "openhearth";
...
function resolveSettings(): ResolvedSettings {
  let cfg: OpenHearthConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => OpenHearthConfig;
      };
