...
import { resolveOpenHearthPackageRoot } from "../infra/openhearth-root.js";
...
const STEP_LABELS: Record<string, string> = {
  ...
  "openhearth doctor entry": "Checking doctor entrypoint",
  "openhearth doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "openhearth";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENHEARTH_REPO_URL = "https://github.com/kylebeee/openhearth.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("openhearth@")) {
    return trimmed.slice("openhearth@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "openhearth.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("openhearth completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.OPENHEARTH_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", OPENHEARTH_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENHEARTH_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENHEARTH_GIT_DIR to an empty folder or an openhearth checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`OPENHEARTH_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveOpenHearthPackageRoot({
  ...
  defaultRuntime.log(theme.heading("OpenHearth update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating OpenHearth..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this OpenHearth install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openhearth doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openhearth gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g openhearth@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g openhearth@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.OPENHEARTH_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.OPENHEARTH_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("openhearth gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openhearth doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("openhearth gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openhearth gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `openhearth update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENHEARTH_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENHEARTH_GIT_DIR to an empty folder or an openhearth checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via OPENHEARTH_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenHearth to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openhearth update", "Update a source checkout (git)"],
        ["openhearth update --channel beta", "Switch to beta channel (git + npm)"],
        ["openhearth update --channel dev", "Switch to dev channel (git + npm)"],
        ["openhearth update --tag beta", "One-off update to a dist-tag or version"],
        ["openhearth update --no-restart", "Update without restarting the service"],
        ["openhearth update --json", "Output result as JSON"],
        ["openhearth update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openhearth update wizard", "Interactive update wizard"],
        ["openhearth --update", "Shorthand for openhearth update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openhearth update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openhearth.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhearth update status", "Show channel + version status."],
          ["openhearth update status --json", "JSON output."],
          ["openhearth update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openhearth.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
