...
Tlon is a decentralized messenger built on Urbit. OpenHearth connects to your Urbit ship and can
...
```bash
openhearth plugins install @openhearth/tlon
```

Local checkout (when running from a git repo):

```bash
openhearth plugins install ./extensions/tlon
```
...
Use these with `openhearth message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenHearth replies in-thread.
