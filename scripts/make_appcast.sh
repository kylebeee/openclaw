...
ZIP=${1:?"Usage: $0 OpenHearth-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/kylebeee/openhearth/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^OpenHearth-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/kylebeee/openhearth/releases/download/v${VERSION}/"}
