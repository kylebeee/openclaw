...
description: "Complete field-by-field reference for ~/.kylebeee/openhearth.json"
---

# Configuration Reference

Every field available in `~/.kylebeee/openhearth.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — OpenHearth uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.openhearth/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `openhearth doctor` into `whatsapp/default`.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupEnabled: false,
        groupChannels: ["openhearth-dm"],
      },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-openhearth",
          requireMention: false,
...
Mattermost ships as a plugin: `openhearth plugins install @openhearth/mattermost`.
...
OpenHearth spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@openhearth", "openhearth"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@openhearth"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `openhearth.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, OpenHearth auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/openhearth" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, OpenHearth assumes `anthropic` (deprecated).
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.openhearth/sandboxes",
        docker: {
          image: "openhearth-sandbox:bookworm-slim",
          containerPrefix: "openhearth-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "openhearth-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "openhearth-sandbox-browser:bookworm-slim",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
- `none`: per-scope sandbox workspace under `~/.openhearth/sandboxes`
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.openhearth/workspace",
        agentDir: "~/.openhearth/agents/main/agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.openhearth/workspace-home" },
      { id: "work", workspace: "~/.openhearth/workspace-work" },
    ],
  },
  bindings: [
...
```json5
{
  session: {
    ...
    store: "~/.openhearth/agents/{agentId}/sessions/sessions.json",
    maintenance: {
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.openhearth/settings/tts.json",
      elevenlabs: {
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:openhearth` | All built-in tools (excludes provider plugins)                                           |
...
OpenHearth uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.openhearth/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `OPENHEARTH_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `openhearth onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `openhearth onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `openhearth onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `openhearth onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `openhearth onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `openhearth onboard --auth-choice minimax-api`.
...
- Loaded from `~/.openhearth/extensions`, `<workspace>/.openhearth/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or OPENHEARTH_GATEWAY_PASSWORD
      allowTailscale: true,
      rateLimit: {
        maxAttempts: 10,
        windowMs: 60000,
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `OPENHEARTH_GATEWAY_PORT` > `gateway.port` > `18789`.
...
```bash
OPENHEARTH_CONFIG_PATH=~/.openhearth/a.json \
OPENHEARTH_STATE_DIR=~/.openhearth-a \
openhearth gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.openhearth-dev` + port `19001`), `--profile <name>` (uses `~/.openhearth-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.openhearth/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-openhearth-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `OPENHEARTH_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.openhearth/workspace/canvas",
    port: 18793,
    liveReload: true,
    // enabled: false, // or OPENHEARTH_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__openhearth__/a2ui/`.
...
- Hostname defaults to `openhearth`. Override with `OPENHEARTH_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.openhearth/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `openhearth dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.openhearth/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${OPENHEARTH_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.openhearth/credentials/oauth.json`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/kylebeee/openhearth.log",
    consoleLevel: "info",
...
- Default log file: `/tmp/kylebeee/openhearth-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `openhearth doctor --fix` can strip unknown keys).
