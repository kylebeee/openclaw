...
Matrix is an open, decentralized messaging protocol. OpenHearth connects as a Matrix **user**
...
```bash
openhearth plugins install @openhearth/matrix
```

Local checkout (when running from a git repo):

```bash
openhearth plugins install ./extensions/matrix
```

If you choose Matrix during configure/onboarding and a git checkout is detected,
OpenHearth will offer the local install path automatically.
...
1. Install the Matrix plugin:
   - From npm: `openhearth plugins install @openhearth/matrix`
   - From a local checkout: `openhearth plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenHearth calls the same
     login endpoint, stores the access token in `~/.openhearth/credentials/matrix/credentials.json`,
...
- On first connection, OpenHearth requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenHearth logs a warning.
...
`~/.openhearth/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openhearth pairing list matrix`
  - `openhearth pairing approve matrix <CODE>`
...
- On startup, OpenHearth resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
