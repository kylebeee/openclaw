...
describe("loadWorkspaceSkillEntries", () => {
  it("handles an empty managed skills dir without throwing", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-"));
  ...
  it("includes plugin-shipped skills when the plugin is enabled", async () => {
    ...
    const pluginRoot = path.join(workspaceDir, ".openhearth", "extensions", "open-prose");

    await fs.mkdir(path.join(pluginRoot, "skills", "prose"), { recursive: true });
    await fs.writeFile(
      path.join(pluginRoot, "openhearth.plugin.json"),
      JSON.stringify(
        {
          id: "open-prose",
          skills: ["./skills"],
