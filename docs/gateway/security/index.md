...
## Quick check: `openhearth security audit`
...
```bash
openhearth security audit
openhearth security audit --deep
openhearth security audit --fix
```
...
- Tighten local perms (`~/.openhearth` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

OpenHearth is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, OpenHearth also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.openhearth/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile`
- **Discord bot token**: config/env (token file not yet supported)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**: `~/.openhearth/credentials/<channel>-allowFrom.json`
- **Model auth profiles**: `~/.openhearth/agents/<agentId>/agent/auth-profiles.json`
- **Legacy OAuth import**: `~/.openhearth/credentials/oauth.json`
...
`openhearth security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${OPENHEARTH_GATEWAY_PASSWORD}
```
...
OpenHearth stores session transcripts on disk under `~/.openhearth/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.openhearth` (see the audit section below). If you need
...
OpenHearth can refresh the skills list mid-session:
...
- If you install plugins from npm (`openhearth plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.openhearth/extensions/<pluginId>/` (or `$OPENHEARTH_STATE_DIR/extensions/<pluginId>/`).
  - OpenHearth uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
openhearth pairing list <channel>
openhearth pairing approve <channel> <code>
```
...
By default, OpenHearth routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
OpenHearth has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.openhearth/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.openhearth or your logs.”
...
4. **Re-run audit**
   - `openhearth security audit --deep` and confirm the report is clean.
...
- `~/.kylebeee/openhearth.json`: `600` (user read/write only)
- `~/.openhearth`: `700` (user only)

`openhearth doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `OPENHEARTH_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_openhearth-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `OPENHEARTH_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `openhearth doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `OPENHEARTH_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `OPENHEARTH_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), OpenHearth
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. OpenHearth verifies the identity by resolving the
...
- OpenHearth will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.openhearth/` (or `$OPENHEARTH_STATE_DIR/`) may contain secrets or private data:

- `openhearth.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `openhearth status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@openhearth", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.openhearth/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `openhearth` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only OpenHearth clients can connect.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.openhearth/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.openhearth/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.openhearth/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `openhearth gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `OPENHEARTH_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/kylebeee/openhearth-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.openhearth/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + OpenHearth version
...
Found a vulnerability in OpenHearth? Please report responsibly:

1. Email: [security@openhearth.ai](mailto:security@openhearth.ai)
