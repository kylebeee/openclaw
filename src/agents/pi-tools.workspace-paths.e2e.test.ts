...
import { createOpenHearthCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("openhearth-ws-", async (workspaceDir) => {
      await withTempDir("openhearth-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createOpenHearthCodingTools({ workspaceDir });
    ...
    await withTempDir("openhearth-ws-", async (workspaceDir) => {
      await withTempDir("openhearth-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello openhearth");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("openhearth-ws-", async (workspaceDir) => {
      const tools = createOpenHearthCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("openhearth-ws-", async (workspaceDir) => {
      await withTempDir("openhearth-override-", async (overrideDir) => {
        const tools = createOpenHearthCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("openhearth-sandbox-", async (sandboxDir) => {
      await withTempDir("openhearth-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "openhearth-sbx-test",
        ...
        const tools = createOpenHearthCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
