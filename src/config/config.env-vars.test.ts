...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.openhearth/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          OPENHEARTH_STATE_DIR: path.join(home, ".openhearth"),
          CLAWDBOT_STATE_DIR: undefined,
          OPENHEARTH_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          OPENHEARTH_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
