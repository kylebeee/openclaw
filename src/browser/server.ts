...
function isAuthorizedBrowserRequest(
  ...
  if (auth.password) {
    const passwordHeader = firstHeaderValue(req.headers["x-openhearth-password"]).trim();
...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.error(`openhearth browser server failed to bind 127.0.0.1:${port}: ${String(err)}`);
    return null;
  });
...
export async function stopBrowserControlServer(): Promise<void> {
  ...
  try {
    ...
    logServer.warn(`openhearth browser stop failed: ${String(err)}`);
  }
