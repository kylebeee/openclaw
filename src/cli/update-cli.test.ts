...
vi.mock("../infra/openhearth-root.js", () => ({
  resolveOpenHearthPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveOpenHearthPackageRoot } = await import("../infra/openhearth-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveOpenHearthPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OpenHearth update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-update-"));
    try {
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "openhearth", version: "1.0.0" }),
        "utf-8",
      );
      ...
      vi.mocked(resolveOpenHearthPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("requires confirmation on downgrade when non-interactive", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        path.join(tempDir, "package.json"),
        JSON.stringify({ name: "openhearth", version: "2.0.0" }),
        "utf-8",
      );
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-update-wizard-"));
    const previousGitDir = process.env.OPENHEARTH_GIT_DIR;
    try {
      setTty(true);
      process.env.OPENHEARTH_GIT_DIR = tempDir;
      ...
      process.env.OPENHEARTH_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
