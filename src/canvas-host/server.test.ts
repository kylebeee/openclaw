...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("openhearthCanvasA2UIAction");
    expect(out).toContain("openhearthSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("openhearthSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.openhearthA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__openhearth__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("openhearth-a2ui-host");
      expect(html).toContain("openhearthCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__openhearth__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("openhearthA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
