...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.OPENHEARTH_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.OPENHEARTH_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.OPENHEARTH_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.OPENHEARTH_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.OPENHEARTH_PROFILE,
      version: environment?.OPENHEARTH_SERVICE_VERSION ?? env.OPENHEARTH_SERVICE_VERSION,
    });
