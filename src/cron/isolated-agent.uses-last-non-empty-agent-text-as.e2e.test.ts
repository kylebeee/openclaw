...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openhearth-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".openhearth", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OpenHearthConfig> = {},
): OpenHearthConfig {
  const base: OpenHearthConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".openhearth", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
