...
describe("resolveStorePath", () => {
  ...
  it("uses OPENHEARTH_HOME for tilde expansion", () => {
    vi.stubEnv("OPENHEARTH_HOME", "/srv/openhearth-home");
    vi.stubEnv("HOME", "/home/other");

    const resolved = resolveStorePath("~/.openhearth/agents/{agentId}/sessions/sessions.json", {
      agentId: "research",
    });

    expect(resolved).toBe(
      path.resolve("/srv/openhearth-home/.openhearth/agents/research/sessions/sessions.json"),
    );
  });
});
...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/openhearth/agents/main/sessions";
  ...
  it("accepts absolute sessionFile paths that resolve within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/openhearth/agents/main/sessions/abc-123.jsonl" },
      { sessionsDir },
    );
  ...
  it("accepts absolute sessionFile with topic suffix within the sessions dir", () => {
    ...
    const resolved = resolveSessionFilePath(
      "sess-1",
      { sessionFile: "/tmp/openhearth/agents/main/sessions/abc-123-topic-42.jsonl" },
      { sessionsDir },
    );
  ...
  it("rejects absolute sessionFile paths outside the sessions dir", () => {
    ...
    expect(() =>
      resolveSessionFilePath(
        "sess-1",
        { sessionFile: "/tmp/openhearth/agents/work/sessions/abc-123.jsonl" },
        { sessionsDir },
      ),
    ).toThrow(/within sessions directory/);
  });
