...
/**
 * Cleanup suspended OpenHearth CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: OpenHearthConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-cli-images-"));
  const paths: string[] = [];
