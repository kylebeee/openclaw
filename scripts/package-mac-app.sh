...
# Build and bundle OpenHearth into a minimal .app we can open.
# Outputs to dist/OpenHearth.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenHearth.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenHearth"
BUNDLE_ID="${BUNDLE_ID:-ai.openhearth.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/kylebeee/openhearth/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenHearth/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenHearthBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenHearthGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenHearth"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenHearth"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenHearth"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenHearth" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenHearth/Resources/OpenHearth.icns" "$APP_ROOT/Contents/Resources/OpenHearth.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenHearth/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenHearthKit resources"
OPENHEARTHKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenHearthKit_OpenHearthKit.bundle"
if [ -d "$OPENHEARTHKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenHearthKit_OpenHearthKit.bundle"
  cp -R "$OPENHEARTHKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenHearthKit_OpenHearthKit.bundle"
else
  echo "WARN: OpenHearthKit resource bundle not found at $OPENHEARTHKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenHearth"
killall -q OpenHearth 2>/dev/null || true
