...
type WebSearchConfig = NonNullable<OpenHearthConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OpenHearthConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("openhearth configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.openhearth.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://openhearth.ai",
      "X-Title": "OpenHearth Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
