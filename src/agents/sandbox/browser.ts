...
import {
  ...
  DEFAULT_OPENHEARTH_BROWSER_COLOR,
  DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME,
...
function buildSandboxBrowserResolvedConfig(params: {
  ...
  return {
    ...
    color: DEFAULT_OPENHEARTH_BROWSER_COLOR,
    executablePath: undefined,
    headless: params.headless,
    noSandbox: false,
    attachOnly: true,
    defaultProfile: DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME,
    profiles: {
      [DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME]: {
...
export async function ensureSandboxBrowser(params: {
  ...
  if (!state.exists) {
    ...
    const args = buildSandboxCreateArgs({
      ...
      labels: { "openhearth.sandboxBrowser": "1" },
    });
    ...
    args.push("-e", `OPENHEARTH_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
    args.push(
      "-e",
      `OPENHEARTH_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`,
    );
    args.push("-e", `OPENHEARTH_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
    args.push("-e", `OPENHEARTH_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
    args.push("-e", `OPENHEARTH_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
  ...
  const existingProfile = existing
    ? resolveProfile(existing.bridge.state.resolved, DEFAULT_OPENHEARTH_BROWSER_PROFILE_NAME)
    : null;
  const shouldReuse =
