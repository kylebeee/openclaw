...
export async function updateConfig(
  mutator: (cfg: OpenHearthConfig) => OpenHearthConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenHearthConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenHearthConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openhearth agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
