...
import type { OpenHearthConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenHearthConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenHearthConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenHearthSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenHearthConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenHearthConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenHearthConfig,
  };
}
