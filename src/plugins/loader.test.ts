...
import { loadOpenHearthPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.OPENHEARTH_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openhearth-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openhearth.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.OPENHEARTH_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadOpenHearthPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.OPENHEARTH_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadOpenHearthPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadOpenHearthPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.OPENHEARTH_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
