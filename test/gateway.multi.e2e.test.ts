...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openhearth-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openhearth");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENHEARTH_STATE_DIR: stateDir,
          OPENHEARTH_GATEWAY_TOKEN: "",
          OPENHEARTH_GATEWAY_PASSWORD: "",
          OPENHEARTH_SKIP_CHANNELS: "1",
          OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENHEARTH_SKIP_CANVAS_HOST: "1",
        },
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-openhearth-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-openhearth-token": gwB.hookToken },
        ),
      ]);
