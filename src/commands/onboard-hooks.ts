...
export async function setupInternalHooks(
  ...
  await prompter.note(
    [
      ...
      "",
      "Learn more: https://docs.openhearth.ai/hooks",
    ].join("\n"),
    "Hooks",
  );
  ...
  await prompter.note(
    [
      ...
      `  ${formatCliCommand("openhearth hooks list")}`,
      `  ${formatCliCommand("openhearth hooks enable <name>")}`,
      `  ${formatCliCommand("openhearth hooks disable <name>")}`,
    ].join("\n"),
    "Hooks Configured",
  );
