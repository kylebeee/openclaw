...
/**
 * Test for issue #6070:
 * `openhearth config set` should use snapshot.parsed (raw user config) instead of
...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-config-cli-"));
  const originalEnv = { ...process.env };
  try {
    // Override config path to use temp directory
    process.env.OPENHEARTH_CONFIG_PATH = path.join(home, ".openhearth", "openhearth.json");
