...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenHearthAgentDir: vi.fn().mockReturnValue("/tmp/openhearth-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openhearth-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openhearth-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenHearthAgentDir: mocks.resolveOpenHearthAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenHearthAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openhearth-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/openhearth-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/openhearth-agent-custom");
