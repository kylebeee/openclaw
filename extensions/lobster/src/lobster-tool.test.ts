...
import type {
  OpenHearthPluginApi,
  OpenHearthPluginToolContext,
} from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "openhearth-lobster-plugin-") {
...
function fakeApi(overrides: Partial<OpenHearthPluginApi> = {}): OpenHearthPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(
  overrides: Partial<OpenHearthPluginToolContext> = {},
): OpenHearthPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("tolerates noisy stdout before the JSON envelope", async () => {
    ...
    const { dir } = await writeFakeLobsterScript(
      `const payload = ${JSON.stringify(payload)};\n` +
        ...
        `process.stdout.write(JSON.stringify(payload));\n`,
      "openhearth-lobster-plugin-noisy-",
    );
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: OpenHearthPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
