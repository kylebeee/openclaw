...
- `gateway`/`node` approvals are controlled by `~/.openhearth/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  OpenHearth prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
