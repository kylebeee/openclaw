...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# OpenHearth Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(openhearth completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# OpenHearth Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(openhearth completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $OPENHEARTH_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "openhearth") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c openhearth -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'openhearth gateway' and not 'openhearth other gateway', we need to check the sequence.
