...
function setTempHome(base: string) {
  ...
  process.env.OPENHEARTH_STATE_DIR = path.join(base, ".openhearth");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "openhearth-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openhearth", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
