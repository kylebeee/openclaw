...
function resolveMode(cfg: OpenHearthConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenHearth doctor");

  const root = await resolveOpenHearthPackageRoot({
  ...
  let cfg: OpenHearthConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("openhearth configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openhearth config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openhearth setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openhearth doctor --fix")}" to apply changes.`);
  }
