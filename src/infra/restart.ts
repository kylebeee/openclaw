...
export function triggerOpenHearthRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENHEARTH_SYSTEMD_UNIT,
        process.env.OPENHEARTH_PROFILE,
      );
  ...
  const label =
    process.env.OPENHEARTH_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENHEARTH_PROFILE);
