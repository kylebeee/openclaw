...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.OPENHEARTH_STATE_DIR;
  previousConfigPath = process.env.OPENHEARTH_CONFIG_PATH;
  previousSkipBrowserControl = process.env.OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.OPENHEARTH_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.OPENHEARTH_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.OPENHEARTH_BUNDLED_PLUGINS_DIR;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENHEARTH_STATE_DIR = path.join(tempHome, ".openhearth");
  delete process.env.OPENHEARTH_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENHEARTH_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENHEARTH_SKIP_CANVAS_HOST = "1";
  process.env.OPENHEARTH_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openhearth-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "openhearth-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".openhearth-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) {
      ...
      process.env.OPENHEARTH_STATE_DIR = previousStateDir;
    }
    if (previousConfigPath === undefined) {
      delete process.env.OPENHEARTH_CONFIG_PATH;
    } else {
      process.env.OPENHEARTH_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.OPENHEARTH_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.OPENHEARTH_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENHEARTH_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.OPENHEARTH_SKIP_CANVAS_HOST;
    } else {
      process.env.OPENHEARTH_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
...
export async function startServerWithClient(
  ...
  const prev = process.env.OPENHEARTH_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENHEARTH_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.OPENHEARTH_GATEWAY_PASSWORD;
