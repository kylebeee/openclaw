...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenHearth status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openhearth security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openhearth security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openhearth.ai/faq");
  runtime.log("Troubleshooting: https://docs.openhearth.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openhearth status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openhearth logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openhearth status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openhearth gateway probe")}`);
  }
}
