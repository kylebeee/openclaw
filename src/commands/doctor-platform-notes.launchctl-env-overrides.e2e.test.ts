...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "OPENHEARTH_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("OPENHEARTH_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv OPENHEARTH_GATEWAY_TOKEN");
    expect(message).not.toContain("OPENHEARTH_GATEWAY_PASSWORD");
  });
