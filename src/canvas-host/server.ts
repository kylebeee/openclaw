...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.openhearthCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.openhearthCanvasA2UIAction &&
        typeof window.openhearthCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.openhearthSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("openhearth:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OpenHearth node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.openhearthSendUserAction === "function"
        ? window.openhearthSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENHEARTH_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.OPENHEARTH_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OpenHearth Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
