...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.OPENHEARTH_SKIP_CHANNELS;
    prevSkipGmail = process.env.OPENHEARTH_SKIP_GMAIL_WATCHER;
    process.env.OPENHEARTH_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.OPENHEARTH_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.OPENHEARTH_SKIP_GMAIL_WATCHER;
    } else {
      process.env.OPENHEARTH_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
