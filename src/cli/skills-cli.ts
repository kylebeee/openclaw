...
function appendHearthHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
  return `${output}\n\nTip: use \`npx hearthub\` to search, install, and sync skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("openhearth skills list")}\` to see all skills.`
      : "No skills found.";
    return appendHearthHubHint(message, opts.json);
  }
  ...
  return appendHearthHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendHearthHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("openhearth skills list")}\` to see available skills.`,
      opts.json,
    );
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.openhearth.ai/cli/skills")}\n`,
    );
