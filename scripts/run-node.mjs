...
const shouldBuild = () => {
  if (env.OPENHEARTH_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message) => {
  if (env.OPENHEARTH_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[openhearth] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["openhearth.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
