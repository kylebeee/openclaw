...
const execHostEnforced = process.env.OPENHEARTH_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.OPENHEARTH_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureOpenHearthCliOnPath({ pathEnv: process.env.PATH ?? "" });
