...
import { readConfigFileSnapshot, type OpenHearthConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: OpenHearthConfig, model: string): OpenHearthConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("openhearth plugins install")}\`.`,
    );
  }
