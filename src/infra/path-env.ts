...
function candidateBinDirs(opts: EnsureOpenHearthPathOpts): string[] {
  ...
  // Bundled macOS app: `openhearth` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openhearth");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/openhearth` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openhearth"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openhearth` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenHearthCliOnPath(opts: EnsureOpenHearthPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENHEARTH_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.OPENHEARTH_PATH_BOOTSTRAPPED = "1";
