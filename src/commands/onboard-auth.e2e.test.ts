...
const requireAgentDir = () => {
  const agentDir = process.env.OPENHEARTH_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENHEARTH_AGENT_DIR not set");
  }
  return agentDir;
};

describe("writeOAuthCredentials", () => {
  const previousStateDir = process.env.OPENHEARTH_STATE_DIR;
  ...
  afterEach(async () => {
    ...
    delete process.env.OPENHEARTH_OAUTH_DIR;
  });

  it("writes auth-profiles.json under OPENHEARTH_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-oauth-"));
    process.env.OPENHEARTH_STATE_DIR = tempStateDir;
    process.env.OPENHEARTH_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENHEARTH_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENHEARTH_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhearth-minimax-"));
    process.env.OPENHEARTH_STATE_DIR = tempStateDir;
    process.env.OPENHEARTH_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.OPENHEARTH_AGENT_DIR;
