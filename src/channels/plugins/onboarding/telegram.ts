...
function setTelegramDmPolicy(cfg: OpenHearthConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
async function noteTelegramUserIdHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      `1) DM your bot, then read from.id in \`${formatCliCommand("openhearth logs --follow")}\` (safest)`,
