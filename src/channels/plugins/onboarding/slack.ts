...
function setSlackDmPolicy(cfg: OpenHearthConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "OpenHearth";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for OpenHearth`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: OpenHearthConfig, allowFrom: string[]): OpenHearthConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
