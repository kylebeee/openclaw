...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(
      `Service not installed. Run: ${formatCliCommand("openhearth gateway install", env)}`,
    );
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const unit = resolveGatewaySystemdServiceName(env.OPENHEARTH_PROFILE);
      hints.push(`Logs: journalctl --user -u ${unit}.service -n 200 --no-pager`);
    } else if (process.platform === "win32") {
      const task = resolveGatewayWindowsTaskName(env.OPENHEARTH_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("openhearth gateway install", env),
    formatCliCommand("openhearth gateway", env),
  ];
