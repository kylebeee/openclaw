...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenHearthAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openhearth models auth setup-token")}\` or \`${formatCliCommand("openhearth configure")}\`.`
          : `Run \`${formatCliCommand("openhearth configure")}\` or set an API key env var.`;
