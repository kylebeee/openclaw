...
function withLoopbackBrowserAuth(
  ...
  if (headers.has("authorization") || headers.has("x-openhearth-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.token) {
      ...
      headers.set("x-openhearth-password", auth.password);
    }
  } catch {
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the OpenHearth gateway (OpenHearth.app menubar, or \`${formatCliCommand("openhearth gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the OpenHearth browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the OpenHearth browser control service. ${hint} (${msg})`);
}
